...
//        }
        ...
        node.clearChildren();
        node.addChild(new FixedExtentBVHNode(extent1));
        node.addChild(new FixedExtentBVHNode(extent2));

        for (BVHNode newChild : node.getChildren()) {
            for (BVHNode previousChild : previousChildrenOfNode) {
                if (previousChild.getExtent().isIntersectingOtherExtent(newChild.getExtent())) {
